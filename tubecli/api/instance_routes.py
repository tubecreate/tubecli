"""Danh tính của máy trong TubeCLI Cloud: tài khoản chủ + số server (17/9/2026).

Cloud PUT sau mỗi lần đăng nhập hộ (lib/tubecli.js → pushCloudIdentity); task video lưu lên Google Drive
đặt thư mục cha theo đó — «<username>-vps-<server_id>/<tên project>» (core/cloud_identity.py).
...
async def put_cloud_identity(req: CloudIdentityRequest, request: Request):
    ...
    try:
        ident = cloud_identity.save(req.username, req.server_id)
