MESSAGES = {
    ...
    "panel.dashboard_manual": "\n[green]Dashboard đang chạy.[/green] Mở đường dẫn này trên trình duyệt:\n\n    [bold cyan]{url}[/bold cyan]\n\n[dim](Máy này không có trình duyệt đồ hoạ nên không tự mở được.)[/dim]",
    "panel.dashboard_container_hint": "[yellow]Trong container thì 127.0.0.1 là loopback của chính container.[/yellow]\n[yellow]Từ máy của bạn, hãy chạy container với[/yellow] [cyan]-p {port}:{port}[/cyan] [yellow]rồi mở[/yellow] [cyan]http://localhost:{port}/dashboard[/cyan]",
    ...
    "panel.ollama_not_installed_short": "\n[red]Ollama chưa được cài — đây là cách chạy model AI ngay trên máy, miễn phí, không cần API key.[/red]",
    "panel.ollama_download_page": "  Trang tải: [cyan]https://ollama.com/download[/cyan]",
    ...
    "panel.docs_manual": "Mở tệp này bằng trình duyệt:\n    [cyan]{path}[/cyan]",
    ...
    "init.setup_done_no_menu": "\n[green]Workspace đã sẵn sàng.[/green]\n  Khởi động server:  [cyan]tubecli api start[/cyan]\n  Rồi mở:            [cyan]http://127.0.0.1:{port}/dashboard[/cyan]\n  Bảng điều khiển:   [cyan]tubecli init[/cyan]\n",
    "wizard.ollama_not_ready":"[yellow]Ollama chưa được cài. Chọn mục 4 ở menu chính — nó sẽ cài Ollama giúp bạn rồi cho chọn model.[/yellow]",
    ...
    "brain.no_api_key": "⚠️ Chưa có API key cho `{model}` nên không gửi được yêu cầu.\n\nThêm key ở Dashboard → Cloud API Keys (http://127.0.0.1:5295), hoặc chạy `tubecli init` rồi chọn \"Cloud API Keys\". Sau đó chọn lại model trong Chat.\n\nKhông muốn dùng key cloud? Cài Ollama (https://ollama.com) rồi chạy `ollama pull qwen:latest` để chạy model ngay trên máy, miễn phí.",
    ...
    "ollama.running_cmd": "\n[cyan]Đang chạy:[/cyan] [yellow]{cmd}[/yellow]",
    ...
    # ── Màn hình "server đã sẵn sàng" (`tubecli info`, cuối cài đặt headless) ──
    "server.ready_title": "  [bold]TubeCLI đã cài xong[/bold]",
    "server.check_ok": "  Kiểm tra:  [green]OK — đang trả lời trên cổng {port}[/green]",
    "server.check_starting": "  Kiểm tra:  [yellow]đang khởi động — một phút nữa xem lại bằng: tubecli info[/yellow]",
    "server.check_down": "  Kiểm tra:  [red]KHÔNG TRẢ LỜI[/red] — xem:  journalctl -u tubecli -n 50",
    "server.check_container": "  Không có systemd (container?). Chạy server bằng:  [cyan]tubecli api start[/cyan]",
    "server.systemd_note": "  Chạy như dịch vụ systemd: đóng SSH vẫn sống, khởi động lại máy tự bật.",
    "server.url_public_note": "      [dim]Đây là địa chỉ công khai của máy chủ — mở từ máy tính của bạn.[/dim]",
    "server.url_nat_note": "      [dim]Thay <ip-máy-chủ> bằng IP công khai của máy — xem trong trang quản lý\n      của nhà cung cấp VPS (bản thân máy chỉ thấy: {private}).[/dim]",
    "server.firewall_note": "\n  [yellow]Nếu trang không mở được: vào trang quản lý VPS mở cổng TCP {port}\n  trong firewall / security group.[/yellow]",
    "server.password_default": "  Mật khẩu đăng nhập:  [bold]123456[/bold]  (mặc định)\n      [yellow]Ai biết địa chỉ này đều đăng nhập được — nên đổi:  tubecli password[/yellow]",
    "server.password_set": "  Mật khẩu đăng nhập:  đã đặt  (đổi lúc nào cũng được:  tubecli password)",
    "server.update_ok": "  Mã nguồn: mới nhất.",
    "server.update_stale": "  [yellow]Mã nguồn: CHƯA cập nhật — có sửa đổi cục bộ chặn update. Gỡ: git -C ~/tubecli stash[/yellow]",
    "server.version_line": "  Phiên bản: {version}",
    "server.commands_title": "  Các lệnh cần nhớ:",
    "server.cmd_password": "      Đổi mật khẩu:      tubecli password",
    "server.cmd_logs": "      Xem log:           tubecli service logs -f",
    "server.cmd_logs_container": "      Xem log:           tubecli api start (chạy trực tiếp)",
    "server.cmd_update": "      Cập nhật:          chạy lại lệnh cài đặt",
    "server.cmd_info": "      Xem lại màn này:   tubecli info",

    # ── Hoàn tất cài đặt headless ──
    "server.setup_title": "\n[bold cyan]Hoàn tất thiết lập máy chủ...[/bold cyan]",
    "server.lang_kept": "  Ngôn ngữ: [green]{lang}[/green]  (đổi bằng: tubecli init --lang xx)",
    "server.pw_prompt": "Đặt mật khẩu bảng điều khiển (Enter = giữ mặc định 123456)",
    "server.pw_kept_default": "  [yellow]Giữ mật khẩu mặc định 123456 — đổi sau bằng: tubecli password[/yellow]",
    "server.pw_changed": "  [green]Đã đặt mật khẩu bảng điều khiển.[/green]",
    "server.pw_env_invalid": "  [yellow]TUBECLI_PASSWORD quá ngắn (tối thiểu 6 ký tự) — giữ mặc định.[/yellow]",
    "server.pw_too_short": "  [yellow]Quá ngắn (tối thiểu 6 ký tự) — nhập lại, hoặc Enter để giữ mặc định.[/yellow]",
    "server.svc_installing": "  Cài dịch vụ systemd (sudo có thể hỏi mật khẩu)...",
    "server.svc_ok": "  [green]Dịch vụ đã cài và đang chạy.[/green]",
    "server.svc_failed": "  [yellow]Chưa cài được dịch vụ. Làm sau bằng:  tubecli service install[/yellow]",
