...
// ── Detailed Composite Furniture Builders ──
const _m = (c, opts = {}) => new THREE.MeshStandardMaterial({ color: c, roughness: opts.r || 0.5, metalness: opts.m || 0.1, ...opts });

function _buildDetailedDesk(g, def) {
    const wood = _m(def.color || '#f0ebe4');
    const metal = _m('#444444', { r: 0.3, m: 0.6 });
    const accent = _m('#333333', { r: 0.4, m: 0.3 });
    // Table top with rounded feel (slightly larger)
...
function _buildDetailedChair(g, def) {
    const fabric = _m(def.color || '#2d3250');
    const metal = _m('#555555', { r: 0.3, m: 0.7 });
    const darkFab = _m('#1a1e35');
    // Seat cushion (slightly rounded via scale)
    ...
    [[-0.26, 0.26]].forEach(([side]) => {
        ...
        // other side
        const arm2 = arm.clone(); arm2.position.x = -side; g.add(arm2);
        const armV2 = armV.clone(); armV2.position.x = -side; g.add(armV2);
    });
    // Central pillar
    ...
    for (let i = 0; i < 5; i++) {
        ...
        g.add(spoke);
        // Wheel
        const wheel = new THREE.Mesh(new THREE.SphereGeometry(0.025, 6, 4), _m('#222'));
        wheel.position.set(Math.sin(angle) * 0.26, 0.025, Math.cos(angle) * 0.26);
        g.add(wheel);
    }
}

function _buildDetailedSofa(g, def) {
    const fabric = _m(def.color || '#3d4a8a', { r: 0.7 });
    const darkFab = _m('#2a3570', { r: 0.7 });
    const legM = _m('#333', { m: 0.5 });
    // Base frame
...
function _buildDetailedBookshelf(g, def) {
    const wood = _m(def.color || '#6b4226', { r: 0.7 });
    const darkW = _m('#4a2e18', { r: 0.7 });
    ...
    // Books on shelves (random colors)
    ...
    for (let shelfY of [0.07, 0.55, 1.05, 1.55]) {
        ...
        for (let b = 0; b < numBooks && bx < 0.48; b++) {
            ...
            const book = new THREE.Mesh(new THREE.BoxGeometry(bw, bh, 0.25), _m(bc, { r: 0.8 }));
...
function _buildDetailedMonitor(g, def) {
    const black = _m('#1a1a2e', { r: 0.2, m: 0.4 });
    const screen = _m('#0a0a18', { r: 0.1, m: 0.3 });
    // Screen panel
    ...
    // Bezel (thin frame)
    const bezel = _m('#111');
    const bTop = new THREE.Mesh(new THREE.BoxGeometry(0.72, 0.015, 0.035), bezel);
    bTop.position.set(0, 1.29, 0); g.add(bTop);
    // Stand neck
    const neck = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.22, 0.04), _m('#333', { m: 0.5 }));
    neck.position.set(0, 0.89, 0); g.add(neck);
    // Stand base
    const standBase = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.02, 0.15), _m('#333', { m: 0.5 }));
    standBase.position.set(0, 0.77, 0.02); g.add(standBase);
    // Keyboard
    const kb = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.015, 0.14), _m('#2a2a2a'));
    kb.position.set(0, 0.76, 0.3); g.add(kb);
    // Key indicators (simple lines)
    ...
    // Mouse
    const mouse = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.018, 0.08), _m('#222'));
    mouse.position.set(0.3, 0.77, 0.32); g.add(mouse);
}

function _buildDetailedPlant(g, def) {
    const potColor = _m('#8B4513', { r: 0.8 });
    const soilColor = _m('#3e2723', { r: 0.9 });
    ...
    // Foliage (stacked spheres for bushy look)
    const leafColors = [0x27ae60, 0x2ecc71, 0x1abc9c, 0x16a085];
    const positions = [
        [0, 0.45, 0, 0.14], [0.06, 0.55, 0.04, 0.11], [-0.05, 0.52, -0.04, 0.10],
        [0.03, 0.62, -0.02, 0.08], [-0.03, 0.58, 0.05, 0.09]
    ];
    positions.forEach(([x, y, z, r], i) => {
        const leaf = new THREE.Mesh(
            new THREE.SphereGeometry(r, 6, 5),
            _m(leafColors[i % leafColors.length], { r: 0.8 })
        );
        leaf.position.set(x, y, z); leaf.castShadow = true; g.add(leaf);
    });
    // Stem
    const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.015, 0.25, 4), _m('#2d5016'));
    stem.position.set(0, 0.32, 0); g.add(stem);
}

function _buildDetailedCabinet(g, def) {
    const metal = _m(def.color || '#8a8a8a', { r: 0.4, m: 0.3 });
    const dark = _m('#666', { r: 0.4, m: 0.3 });
    // Main body
    ...
    for (let dy of [0.22, 0.58, 0.94]) {
        ...
        // Handle
        const handle = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.015, 0.02), _m('#444', { m: 0.6 }));
        handle.position.set(0, dy + 0.15, 0.25); g.add(handle);
    }
}

function _buildDetailedWhiteboard(g, def) {
    const frame = _m('#666', { r: 0.3, m: 0.4 });
    const white = _m('#f0f0f0', { r: 0.3 });
    // Board surface
    ...
    [[-0.12, 0xff3333], [0, 0x3333ff], [0.12, 0x33cc33]].forEach(([mx, mc]) => {
        const marker = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.1, 6), _m(mc));
        marker.position.set(mx, 0.99, 0.04); marker.rotation.z = Math.PI / 2; g.add(marker);
    });
}

// Map asset IDs to detail builders
const DETAIL_BUILDERS = {
    desk_modern: _buildDetailedDesk,
    desk_wood: _buildDetailedDesk,
    chair_office: _buildDetailedChair,
    sofa: _buildDetailedSofa,
    bookshelf: _buildDetailedBookshelf,
    monitor: _buildDetailedMonitor,
    plant_pot: _buildDetailedPlant,
    cabinet: _buildDetailedCabinet,
    whiteboard: _buildDetailedWhiteboard,
};

// ── Render Studio Custom Assets (HQ Composite) ──
function renderStudioAssets(sceneAssets) {
    sceneAssets.forEach(item => {
        const def = STUDIO_ASSETS.find(a => a.id === item.asset_id);
        if (!def) return;

        const builder = DETAIL_BUILDERS[item.asset_id];
        if (builder) {
            // Use detailed composite builder
            const g = new THREE.Group();
            g.position.set(item.x || 0, 0, item.z || 0);
