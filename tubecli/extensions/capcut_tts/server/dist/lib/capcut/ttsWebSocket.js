...
exports.synthesizeWithTimestamps = exports.fetchTtsToken = void 0;
 ...
 */
const createTokenSignature = (tokenUrl, platformId, appVersion) => {
 ...
 */
const fetchTtsToken = async (options) => {
    const signature = createTokenSignature(options.tokenUrl, options.platformId, options.appVersion);
    ...
    if (!payload?.data?.token || !payload.data.app_key) {
        throw new Error(`CapCut TTS token request failed: ${payload?.ret ?? '?'} ${payload?.errmsg ?? ''}`);
    }
    return { token: payload.data.token, appKey: payload.data.app_key };
};
exports.fetchTtsToken = fetchTtsToken;
/**
...
const parseMarks = (payload, includePhonemes) => {
    const alignment = isRecord(payload) && isRecord(payload.alignment) ? payload.alignment : null;
    const words = [];
    const phonemes = [];
 ...
 */
const synthesizeWithTimestamps = async (options) => {
    const { token, appKey } = await (0, exports.fetchTtsToken)(options);
    ...
    return new Promise((resolve, reject) => {
        ...
        const finish = (error) => {
            ...
            if (audio.byteLength === 0) {
                reject(new Error('CapCut TTS WebSocket returned no audio. The speaker is likely not supported on this path'));
                return;
            }
            resolve({ audio, text, duration, words, phonemes });
        };
        const timer = setTimeout(() => finish(new Error('CapCut TTS WebSocket timed out')), options.timeoutMs ?? 30_000);
            ...
            if (typeof data === 'string') {
                ...
                if (frame.event === 'TTSResponse' && typeof frame.payload === 'string') {
                    try {
                        ...
                        if (marks.words.length)
                            words = marks.words;
                        if (marks.phonemes.length)
                            phonemes = marks.phonemes;
                    }
                    catch {
                ...
                if (frame.event === 'TaskFailed') {
                    finish(new Error(`CapCut TTS WebSocket task failed: ${String(frame.status_text ?? '')}`));
                }
                return;
            }
            chunks.push(Buffer.from(data instanceof ArrayBuffer ? new Uint8Array(data) : data));
        });
        socket.addEventListener('error', () => finish(new Error('CapCut TTS WebSocket error')));
...
exports.synthesizeWithTimestamps = synthesizeWithTimestamps;
//# sourceMappingURL=ttsWebSocket.js.map