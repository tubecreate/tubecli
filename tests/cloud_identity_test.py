...
Kiểm:
  A. core/cloud_identity: chưa biết → «tubecli-<tên máy>»; lưu → «username-vps-9»; sai dạng → ValueError;
     file hỏng → coi như chưa biết; không đổi thì không ghi lại
...
saved = CI.save("tuan89tk", 9)
ok(saved["username"] == "tuan89tk" and saved["server_id"] == 9 and FILE.is_file(), "lưu → ghi file", saved)
ok(CI.drive_root_name() == "tuan89tk-vps-9", "tên thư mục «username-vps-<số server>»", CI.drive_root_name())
mtime = FILE.stat().st_mtime_ns
seen = CI.load()["seen"]
ok(CI.save("tuan89tk", "9")["seen"] == seen and FILE.stat().st_mtime_ns == mtime, "không đổi gì → không ghi lại file")
ok(CI.save("vinh_it.2", 17)["username"] == "vinh_it.2" and CI.drive_root_name() == "vinh_it.2-vps-17",
   "đổi tài khoản / máy → tên mới")
for bad_u, bad_id, label in (("", 9, "username rỗng"), ("a/b", 9, "username có /"), ("a b", 9, "username có khoảng trắng"),
                             ("x" * 65, 9, "username quá dài"), ("ok_user", 0, "server_id 0"),
                             ("ok_user", -3, "server_id âm"), ("ok_user", "abc", "server_id chữ"),
                             ("ok_user", True, "server_id kiểu bool")):
    try:
        CI.save(bad_u, bad_id)
        ok(False, f"sai dạng phải ném: {label}")
    except ValueError:
        ok(CI.drive_root_name() == "vinh_it.2-vps-17", f"sai dạng → ValueError, giữ danh tính cũ: {label}")
FILE.write_text("{broken", encoding="utf-8")
ok(CI.load() == {} and CI.drive_root_name().startswith("tubecli"), "file hỏng → coi như chưa biết")
FILE.write_text('{"username": "../x", "server_id": 3}', encoding="utf-8")
...
body = {"username": "tuan89tk", "server_id": 9}
...
r = c.put("/api/v1/instance/cloud-identity", json=body)
ok(r.status_code == 200 and r.json()["drive_root"] == "tuan89tk-vps-9" and CI.load()["server_id"] == 9,
   "phiên chủ (cloud đăng nhập hộ) → lưu, trả tên thư mục", r.text)
r = c.put("/api/v1/instance/cloud-identity", json={"username": "a/b", "server_id": 9})
ok(r.status_code == 400 and CI.drive_root_name() == "tuan89tk-vps-9", "tên sai dạng → 400, giữ cũ", r.status_code)
r = c.get("/api/v1/instance/cloud-identity")
ok(r.status_code == 200 and r.json()["identity"]["username"] == "tuan89tk" and r.json()["drive_root"] == "tuan89tk-vps-9",
   "GET: danh tính + tên thư mục", r.text)
